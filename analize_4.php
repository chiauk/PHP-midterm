      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ...
          <?php
				...
				//讀取table資料
				$sql="SELECT `place_num`,count(`place_num`) AS B, sum(`count`) FROM (SELECT place.`place_num`,`count` FROM place,po WHERE place.`place_id`=po.`place_id`)AS A GROUP BY `place_num` ORDER BY B DESC LIMIT 0,5";
